...
class Detector(object):
    ...
    def sef(self, img, sigma, c):
        ...
        img_sef1 = sp.ndimage.gaussian_laplace(
            img_filtered, sigma)  # calculate laplacian of gaussian
    ...
    def radialsym_centre(self, img):
        ...
        fdu = sp.signal.convolve2d(
            dIdu, filter_core,  mode='same', boundary='fill', fillvalue=0)
        fdv = sp.signal.convolve2d(
            dIdv, filter_core,  mode='same', boundary='fill', fillvalue=0)
    ...
    def detect(self, frame):
        ...
        peaks_coor = peak_local_max(
            self.img_sef, min_distance=self.min_distance, threshold_rel=self.threshold_rel)
